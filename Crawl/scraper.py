...
INPUT_FILE = os.path.join(BASE_DIR, "data", "urls", "list_url.txt")
OUTPUT_DIR = os.path.join(BASE_DIR, "data", "markdown")
SCRAPED_TRACKER = os.path.join(BASE_DIR, "data", "scraper_state.txt")

# Pemetaan domain ke CSS Selector
DOMAIN_SELECTORS = {
    "jatim.antaranews.com": "div.wrap__article-detail-content.post-content",
    "antaranews.com": "div.wrap__article-detail-content.post-content",
    ...
    "surabaya.tribunnews.com": "div.side-article.txt-article.multi-fontsize.editcontent",
...
def scrape_url(url):
    ...
    for element in content_elements:
        ...
        if element.name == 'p':
            ...
            # Hapus struktur tag inline agar kalimat tidak terpecah oleh stripped_strings
            for tag in element.find_all(['a', 'span', 'strong', 'b', 'i', 'em']):
                tag.unwrap()
            
            # Ekstrak menggunakan stripped_strings untuk menangkap teks di luar <p> atau di dalam <br>
            for string in element.stripped_strings:
                # Membersihkan spasi berlebih
                clean_string = re.sub(r'\s+', ' ', string).strip()
                if len(clean_string) >= 20 and clean_string not in paragraphs:
...
def main():
    if not os.path.exists(INPUT_FILE):
        print(f"[!] File {INPUT_FILE} tidak ditemukan. Jalankan crawler.py terlebih dahulu.")
        return
